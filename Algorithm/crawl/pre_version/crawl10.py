    ...
    try:
        for keywordset_doc in db.keyword_set.find().sort('survey_index', pymongo.ASCENDING):
            ...
            for link in links:
                ...
                collection2.insert({'title':title,'url':video_src,'img_src':img_src,'date': date, 'keyword_title_id': keyword_title_id,'keyword_title':keyword_title})
                print(title)
                print(video_src)
                print(img_src)

        print({'code': 100, 'msg': "유튜브크롤링이 완료되었습니다."})

    except:
        print({'code': 1, 'msg': "유튜브크롤링오류가 발생하였습니다."})
