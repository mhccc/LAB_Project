...
collection = db.LinkCollection
#DB내용 출력테스트시 5~7번 28~29번 주석처리 필요

def get_google_search(keyword_title, *keyword_list):
    try:
        googleUrl = 'https://www.google.co.kr/search?source=hp&q='
        ...
        for pages in range(0,6): #다음페이지를 위한 for문
            ...
            for title in titles: #각 부분별 크롤링(하나의 링크마다 반복됨)
                try:
                    ...
                    #collection.insert({"title":title.text,"url":link2,"key_score":w3,"date":date})    #findOneAndUpdate오류 발생시 사용
...
if __name__=='__main__':
    ...
    get_google_search('치질','치질에 좋은음식','유방암에 좋은음식')
